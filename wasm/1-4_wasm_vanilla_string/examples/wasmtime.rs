...
fn main() {
  ...
  let resize = instance.get_typed_func::<i32, ()>(&mut store, "resize").unwrap();
  let get_ptr = instance.get_typed_func::<(), i32>(&mut store, "get_ptr").unwrap();
  let get_size = instance.get_typed_func::<(), i32>(&mut store, "get_size").unwrap();
  let memory = instance.get_memory(&mut store, "memory").unwrap();

  println!("current_pages : {}", memory.size(&store)); // memory.grow(&mut store, additional_pages)?;
  ...
  let src = "goodmorning world";
  {
    let _ = resize.call(&mut store, src.len() as i32).unwrap();
    let ptr = get_ptr.call(&mut store, ()).unwrap() as i32;
    let len = get_size.call(&mut store, ()).unwrap() as i32;

    memory.write(&mut store, ptr as usize, len).unwrap();

    let _ = print.call(&mut store, (ptr as i32, len as i32)).unwrap();
  }
